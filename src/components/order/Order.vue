    ...
    <div>
        ...
        <el-card>
         ...
         <el-table :data="orderlist" border stripe>
            ...
            <el-table-column label="订单价格" prop="order_price" type="number"></el-table-column>
      ...
      const { data: res } = await this.$http.put('/orders/' + this.addressForm.order_id,
        {
          ...
          order_number: this.addressForm.order_number,
          pay_status: this.addressForm.pay_status
        })
      if (res.meta.status !== 201) {
        this.$message.error('更新地址失败！')
      }
      const { address1, address2 } = this.addressForm
      // 将修改的地址更新到数据
      res.data.consignee_addr = address1.toString().replace(/,/g, '') + address2
      // console.log(res)
      this.getOrderList()
